...
static void server_init(char *conf) {
	...
	for (i = 0; i < setting.mod_num; i++) {
		...
		if (strlen(mod->init_func_name)) {
			...
			if (mod->init_func(mod->mod_conf)) {
				fprintf(stderr, "module[%s] initialzation error\n", mod->mod_name);
				exit(1);
			}
		} else {
			mod->init_func = NULL;
		...
		if (strlen(mod->proc_func_name)) {
			//mod->proc_func = (int (*)(const req_rsp_pack*, req_rsp_pack*, int))dlsym(mod->mod_id, mod->proc_func_name); 
...
int main(int argc, char *argv[]) {
	...
	int server_sock, c, daemon = 1;
	char conf_file[255], action[255];

	/* default configure file */
	//strcpy(conf_file, "./server.ini");
	strcpy(conf_file, "./Aggregator_Service.ini");
	//strcpy(conf_file, "./Comment_Service.ini");
	//strcpy(conf_file, "./Basic_Service.ini");
	//strcpy(conf_file, "./Suggestion_Service.ini");
