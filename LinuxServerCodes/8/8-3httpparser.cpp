...
enum CHECK_STATE {
    CHECK_STATE_REQUESTLINE = 0, // 正在分析行
...
int main( int argc, char* argv[] )
{
    ...
    else
    {
        ...
        int data_read = 0;
        int read_index = 0; // 当前读取的客户数据 字节数
        int checked_index = 0; // 已经分析完的客户数据 字节数
        ...
        while( 1 )
        {
            ...
            if( result == NO_REQUEST ) // 暂时未得到一个完整的 HTTP 请求
            {
                continue;
            }
            else if( result == GET_REQUEST ) // 得到了一个完整而正确的 HTTP 请求
            {
